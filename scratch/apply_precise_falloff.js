...
const replacement = `    // =========================================================
    ...
    for (let i = 0; i < GIRL_DATA.length; i++) {
      ...
      if (!isKeyPart && distFromCenter > 20) {
        ...
        // 外側へ行くほどドット生存率が急減（2次曲線減衰）
        const keepProb = Math.max(0.02, Math.pow(1.0 - t, 2.2));
        ...
        edgeWeight = Math.max(0.15, Math.pow(1.0 - t, 1.4));
      }
    ...
    // =========================================================
    // 5. 少女の全身完全反転水鏡（外側に向かってドットが減る自然な水鏡）
    // =========================================================
...
console.log('Successfully replaced with smooth falloff!');
