...
// 少女の身体領域（x: -52..52, y: -128..0）は背景粒子が重なってゴミ粒子に見えないようにクリーンに除外
const nebulaPts = [];
for (let y = -307; y <= -1; y += 5) {
  for (let x = -512; x <= 512; x += 5) {
    // 少女の輪郭・身体の領域は背景粒子を除外して清潔に保つ
    if (Math.abs(x) <= 48 && y >= -126 && y <= -2) {
      continue;
    }
...
for (let i = 0; i < 738; i++) {
  ...
  // 少女の身体の中に入り込む星は除外
  if (Math.abs(sx) < 45 && sy > -126 && sy < -2) continue;
...
// 4. GIRL_DATA & GIRL_REFL_DATA
// 輪郭（rim）と髪（loose_hair）は100%完全保持し、身体（body）も均一にサンプリングして途切れや穴あきを完全解消
...
rawGirl.forEach((p, idx) => {
  ...
  // 輪郭と髪は100%すべて配置
  // 身体部分も密に保持（3点に1点均等サンプリングで滑らかに充填）
  if (isKey || (idx % 3 === 0)) {
    girlPts.push({
      bx: x,
      by: y,
...
console.log('Clean Girl Points:', girlPts.length, 'Refl Points:', girlReflPts.length);

// 5. WATER_RIPPLE_PTS（半円アーチ曲線の地平線とさざ波光条）
...
// 6. WATER_STAR_REFL（星々の水鏡反射 674点）
const waterStarRefl = [];
for (let i = 0; i < 674; i++) {
...
console.log('Generating perfected clean stargazer.js! Total count:', totalCount);

const outputCode = `/**
 * stargazer.js - Starry Mirror & The Stargazer (星の鏡と佇む少女) [Clean Silhouette Edition]
 * 
 * 仕様:
 * 1. 配列長は毎フレーム 100% 厳密に固定（総数: ${totalCount} 点）。
 * 2. 少女の輪郭・髪を100%鮮明に保持し、周囲の浮遊ゴミ粒子を完全排除。
...
console.log('Successfully written clean silhouette particle/stargazer.js!');
