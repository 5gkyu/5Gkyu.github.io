...
// 1. NEBULA_DATA（暗夜・星雲グリッド：全域に均一に敷き、四角い切り抜き壁を完全排除）
const nebulaPts = [];
...
// 4. GIRL_DATA & GIRL_REFL_DATA（輪郭・髪・身体の鮮明な実体）
...
rawGirl.forEach((p, idx) => {
  const x = p.rx;
  const y = p.ry;
  const isKey = (p.part === 'rim' || p.part === 'loose_hair');

  let keep = false;
  if (isKey) {
    keep = true;
  } else if (p.lum >= 30) {
    if (idx % 2 === 0) keep = true;
  } else if (Math.abs(x) <= 24 && p.lum >= 22) {
    if (idx % 2 === 0) keep = true;
  }
...
// 5. AURA_GRADIENT_PTS（キャラクターの周囲へ広がり、徐々に減衰する滑らかなオーラグロー）
// これにより、四角い壁境界線を完全排除し、外側へ向けて自然にフェードアウトする光のグラデーションを形成
const auraGradientPts = [];
const auraXSpan = 180;
for (let y = -135; y <= 130; y += 4.2) {
  for (let x = -auraXSpan; x <= auraXSpan; x += 5.2) {
    const absX = Math.abs(x);
    // 少女の中心から外側へ向かう滑らかな減衰率
    const t = absX / auraXSpan;
    const keepProb = Math.pow(1.0 - t, 1.8);
    const hash = Math.abs(Math.sin(x * 12.9898 + y * 78.233) * 43758.5453) % 1;

    if (hash < keepProb * 0.72) {
      const isWater = (y > 0);
      const depthFactor = isWater ? (y / 130.0) : (-y / 135.0);
      const fade = Math.pow(1.0 - t, 1.5);

      const r = Math.round((isWater ? 16 : 14) * fade + 4);
      const g = Math.round((isWater ? 36 : 28) * fade + 8);
      const b = Math.round((isWater ? 85 : 68) * fade + 15);

      auraGradientPts.push({
...
console.log('Aura Gradient Points:', auraGradientPts.length);

// 6. WATER_RIPPLE_PTS（半円アーチ曲線の地平線とさざ波光条）
function getHorizonY(x) {
  const normX = Math.max(-1, Math.min(1, x / 480));
...
for (let x = -480; x <= 480; x += 3.5) {
  ...
  waterRipplePts.push({
    bx: Math.round(x * 10) / 10,
    by: Math.round(hy * 10) / 10,
    size: 1.4,
    type: 'horizon_edge',
    phase: x * 0.05
  });
}
// 水面さざ波ライン（半円アーチに沿って優美に広がる波）
for (let dy = 6; dy <= 126; dy += 5) {
  const yRatio = dy / 126;
  const maxSpan = 480 * (1 - yRatio * 0.12);
  const stepX = 6.0 + yRatio * 3.5;
  for (let x = -maxSpan; x <= maxSpan; x += stepX) {
    const hy = getHorizonY(x);
    const y = hy + dy;
    waterRipplePts.push({
      bx: Math.round(x * 10) / 10,
      ...
      type: 'ripple_line',
      depthRatio: yRatio,
      phase: x * 0.04 + dy * 0.08
    });
  }
}

// 7. WATER_STAR_REFL（星々の水鏡反射 674点）
const waterStarRefl = [];
for (let i = 0; i < 674; i++) {
  const sx = Math.sin(i * 77.3) * 460;
  const hy = getHorizonY(sx);
  const sy = hy + 4 + (Math.cos(i * 44.1) * 0.5 + 0.5) * 115;
  waterStarRefl.push({
    bx: Math.round(sx * 10) / 10,
    by: Math.round(sy * 10) / 10,
    rgb: [200, 235, 255],
...
const totalCount = nebulaPts.length + starsPts.length + majorStarPts.length +
                   girlPts.length + girlReflPts.length + auraGradientPts.length +
                   waterRipplePts.length + waterStarRefl.length + shootSlotCount;

console.log('Generating perfected smooth gradient stargazer.js! Total count:', totalCount);

const outputCode = `/**
 * stargazer.js - Starry Mirror & The Stargazer (星の鏡と佇む少女) [Smooth Gradient Aura Edition]
 * 
 * 仕様:
 * 1. 配列長は毎フレーム 100% 厳密に固定（総数: ${totalCount} 点）。
 * 2. キャラクター周囲の青いオーラが外側に向かって自然にグラデーションで減衰し、境界線の壁を完全排除。
 * 3. 満天の星空・天の川銀河光芒・5種の大星・全身上下反転水鏡・流麗な半円アーチ曲線の水面を描画。
 */
...
(function (root) {
  ...
  const AURA_GRADIENT_PTS = ${JSON.stringify(auraGradientPts)};
...
console.log('Successfully written smooth gradient aura particle/stargazer.js!');
