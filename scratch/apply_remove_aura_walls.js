...
// 輪郭（rim）と髪（loose_hair）は100%完全保持。
// 身体本体（body）は本体実体（lum >= 28）のみを保持し、外側に広がる余分な青いオーラ（霞）を完全排除。
...
console.log('Generating perfected clean aura-free stargazer.js! Total count:', totalCount);

const outputCode = `/**
 * stargazer.js - Starry Mirror & The Stargazer (星の鏡と佇む少女) [Aura-Free Clean Silhouette]
 * 
 * 仕様:
 * 1. 配列長は毎フレーム 100% 厳密に固定（総数: ${totalCount} 点）。
 * 2. 余分な青いオーラ（境界線の壁）を完全排除し、鮮明な少女シルエットと自然な背景グラデーションを実現。
...
console.log('Successfully written clean aura-free particle/stargazer.js!');
