...
// 1. 定義部：さざ波ラインが少女の真下で穴を開けずに自然に通るように修正
const rippleLoopOld = `      // 少女の反射の中心部は間引き、反射像をクリアに際立たせる
      if (x >= -45 && x <= 45 && y >= 1 && y <= 127) {
        if (Math.abs(x) < 32) continue;
      }`;

const rippleLoopNew = `      // 少女の真下も不自然に途切れさせず、水面全体を一本の流麗なさざ波として通す`;

if (code.includes(rippleLoopOld)) {
  code = code.replace(rippleLoopOld, rippleLoopNew);
  console.log('Replaced ripple hole with continuous wave');
}

// 2. 描画ループ：GIRL_DATA（少女本体）と GIRL_REFL_DATA（反射）の両方に
// 「外側に行くにつれてドットが減る（確率的ディザリング＆フェードアウト）」を適用し、
// 長方形の明らかな境界線を完全に排除する！
const girlSectionOld = `    // =========================================================
    // 4. 少女（呼吸モーション ＆ 夜風になびく髪と裾）
    // =========================================================`;

const girlSectionEnd = `// =========================================================
    // 5.1 星々の水面反射（星空の輝きの映り込み）`;

const girlSectionNew = `// =========================================================
    // 4. 少女（呼吸モーション ＆ 外側に向かって自然にドットが減る滑らかな境界）
    // =========================================================
    ...
    for (let i = 0; i < GIRL_DATA.length; i++) {
      ...
      if (!isKeyPart && distFromCenter > 20) {
        const t = Math.min(1.0, (distFromCenter - 20) / 42.0);
        // 生存確率：中心付近は100%、外側に向かって滑らかに減少
        const keepProb = Math.max(0.04, Math.pow(1.0 - t, 1.8));
        ...
        edgeWeight = Math.max(0.3, 1.0 - t * 0.6);
      }
    ...
    for (let i = 0; i < GIRL_REFL_DATA.length; i++) {
      ...
      if (!isKeyPart && distFromCenter > 20) {
        ...
        edgeWeight = Math.max(0.25, 1.0 - t * 0.7);
      }
...
const startIdx = code.indexOf(girlSectionOld);
const endIdx = code.indexOf(girlSectionEnd);

code = code.slice(0, startIdx) + girlSectionNew + code.slice(endIdx);

fs.writeFileSync('particle/stargazer.js', code, 'utf8');
console.log('Successfully eliminated box boundary and added smooth outward dot falloff!');
